...
- **`risk.py`** - VaR (historical + parametric), CVaR, Kupiec + Christoffersen VaR backtests, conditional EVT (McNeil-Frey), **stationary block-bootstrap Monte Carlo** (`monte_carlo`), and Merton jump-diffusion Monte Carlo (`calibrate_jump_diffusion` + `jump_diffusion_mc`). The bootstrap resamples CONTIGUOUS stretches (Politis & Romano 1994), not single days: daily returns are near-uncorrelated in sign but their magnitudes cluster, so i.i.d. resampling scatters a crash week across a hundred simulated years and thins the one-year tail this engine reports as CVaR. The expected block length is measured from the book's own |returns| autocorrelation (`mean_block_length`, integrated autocorrelation time, clamped [2, 40]), never typed in; `block=False` restores i.i.d. for comparison, and the UI states the block length in days. On Mag-7 the switch moved CVaR by ~2.4 points, so it is not cosmetic.
...
- **Support:** `SUPPORT_EMAIL` in `main.py` (project mailbox `meleona.support@gmail.com`, forwarded to the maintainer; override per environment with `MELEONA_SUPPORT_EMAIL`) is the single source - footer, maintenance page and the privacy panel all read it. Users quote the session reference; `RUNBOOK.md` §3 turns it into a traceback.
- **Phased release:** `MELEONA_CHANNEL=beta` shows the beta notice and the prefilled "report this session" link; unset on production. `python -m tools.warm_cache` warms every preset after a deploy, `--drill` times a cold refill (the app's RTO).
...
5. **Aesthetic:** Citadel-style institutional theme - beige `#EDE9E3`/`#D4CDBF`, bronze `#9A7B4F`/`#8A6A3C`, charcoal `#3F3B35`, serif (Georgia). Defined in `.streamlit/config.toml` and **`static/app.css`** (extracted from `main.py` 2026-08-26 and loaded once by `load_css()` - it was 778 lines of inline `<style>` re-sent on every rerun). Keep new UI additions consistent with this (no bright/flashy colors, no sans-serif headings).

   **Text tones differ from decorative tones, deliberately.** Bronze `#9A7B4F` measures 2.50:1 on this beige ground - fine for a hairline rule, illegal for words. Text uses the darker `#6A5030` (beige grounds) and `#C6A26B` (charcoal bands); seven tones were corrected on 2026-08-26. `test_text_colours_meet_wcag_aa` recomputes every ratio from the stylesheet, so a palette edit that drops text below 4.5:1 fails the build. When adding a colour: decorative may stay light, anything carrying words must clear AA on the ground it sits on. See `ACCESSIBILITY.md`.
